...
<script type="text/javascript">
	...
	L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=<? echo($mapbox_api_key); ?>', {
		attribution: '(C) <a href="https://www.mapbox.com/about/maps/">Mapbox</a> (C) <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> <strong><a href="https://www.mapbox.com/map-feedback/" target="_blank">Improve this map</a></strong>',
		tileSize: 512,
		maxZoom: 18,
		zoomOffset: -1,
	...
	<?
		...
		while ($row = mysqli_fetch_array($result, MYSQL_NUM)) {
			...
			echo("marker = L.marker(latLng,{icon:icon[$color]}).on('click',setCurrentMarker).addTo(markersLayer);\n");
			echo("marker.bindPopup('".$label."');\n");
			
			if ($message==$row[14]) {
				echo("openMarker = marker;");
				echo("latLngOpenMarker = L.latLng($lat, $lng);");
				echo("currentMarker = marker;");
				echo("latLngCurrentMarker = L.latLng($lat, $lng);");
			}
		}
		?>	
	...
	function setCurrentMarker(e){
		currentMarker=e;
		latLngCurrentMarker=e.latlng;
	}
	
	ovMap.on('zoom', function(){
		if(currentMarker!=null){
			ovMap.setView(latLngCurrentMarker, ovMap.getZoom());
		}
	});
