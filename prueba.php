...
<script src="./includes/BenzAMRRecorder.min.js"></script>
<script>
	//see https://github.com/BenzLeung/benz-amr-recorder/blob/master/README-EN.md
	class amrPlayer {
		constructor(id, file){
			...
			var thisPlayer = new BenzAMRRecorder();
			
			thisPlayer.initWithUrl(file).then(function(){
				//
			});
		...
		action(){
			...
			thisPlayer.playOrPauseOrResume();
			this.state = 1 - this.state;
		}
	}
	
	function createPlayer(id, file){
		const p = new amrPlayer(id,file);
		amrPlayers.push(p);
	}
	
	function toggleState(myPlayer){
		myPlayer.state = 1 - myPlayer.state;
	}
	
	function doAction(id){
		var myPlayer = amrPlayers.find(p => p.id === id);
		myPlayer.action();
		var myAMRPlayer = myPlayer.player;
		myAMRPlayer.onEnded(function() {
			var is = "audio_" + id;
			const s = document.getElementById(is);
			s.innerHTML='<img src="./includes/images/play-amr.png">';
			toggleState(myPlayer);
		})
		var is = "audio_" + id;
		const s = document.getElementById(is);
		if (myPlayer.state==0){			
			...
			s.innerHTML='<img src="./includes/images/pause-amr.png">';
		}
	}
...
if ($page_filter!="") {
	...
	echo($page_filter);
	echo(" <a href=\"".$main_page."?c=$c&r=1\">".$ov_deselect_tags[$language]."</a></h1>");
}
...
while ($row = mysqli_fetch_array($result, MYSQL_NUM)) {
	...
	$att_result = mysqli_query($dbh,$att_query);
	$prev_type = "";
	$pending = false;
	while ($att_row = mysqli_fetch_array($att_result, MYSQL_NUM)) {
		...
		if($type == 2){
			if($discard_short_audio==true && filesize($filename) < $min_audio_size){
				continue;
			}
			if(strpos($filename,".amr")>0){
?><script>createPlayer(<? echo($message_id); ?>,'<? echo($filename); ?>');</script><?
			}
		}
...
?><audio src="<? echo($filename); ?>" preload="none"></audio><?
						} else if (strpos($filename,".amr")>0){
?><div style="width: <? echo($audio_width); ?>px; height: 36px; background-color: #497A2B; text-align: center;"><a href="#!" onclick="doAction(<? echo($message_id); ?>);"><span id="audio_<? echo($message_id); ?>"><img src="./includes/images/play-amr.png"></span></a></div><?
						}
					} else {
