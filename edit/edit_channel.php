...
<?
	...
	$n_attachments = mysqli_num_rows($att_result);
	$n_check_photo = 0;
	$n_check_audio = 0;
	$n_check_video = 0;
	$n_publish_photo = 0;
...
?><script>createPlayer(<? echo($message_id); ?>,'<? echo($filename); ?>');</script><?
				...
				if(strpos($filename,".mp3")>0){
					...
					$alt="<tr><td valign=\"bottom\"><div style=\"width: ".$edit_audio_width."px; height: 36px; background-color: #497A2B; text-align: center;\"><a href=\"#!\" onclick=\"doAction(".$message_id.");\"><span id=\"audio_".$message_id."\"><img src=\"./../includes/images/play-amr.png\"></span></a></div></td></tr>";
				}
			}
			echo($alt);
		}
