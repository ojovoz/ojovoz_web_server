	...
	if ($inbox = imap_open ($server, $user, $pass)) {
		...
		for($x=1; $x<=$total; $x++) {
			...
			if ($subject=="ojovoz" && is_array($sections) && sizeof($sections)>0) {
				...
				$result =mysqli_query($dbh,$query);
				$current_message = mysqli_insert_id($dbh);
				$lat="";
				$long="";
				for($y=0; $y<sizeof($sections); $y++) {	
					...
					$filename = ($sections[$y]["name"])? $sections[$y]["name"] : $sections[$y]["filename"];
					if (strpos($filename,".jpg") > 0 && $type=="image/jpeg") {
						$subfolder = "/image/image";
						$extension = ".jpg";
						$att_type = 1;
					} else if (strpos($filename,".wav") > 0 && $type=="application/octet-stream") {
						$subfolder = "/sound/sound";
						$extension = ".wav";
						$att_type = 2;
					} else if (strpos($filename,".amr") > 0 && $type=="application/octet-stream") {
						$subfolder = "/sound/sound";
						$extension = ".amr";
						$att_type = 2;
					} else if (strpos($filename,".mp3") > 0 && $type=="application/octet-stream") {
						$subfolder = "/sound/sound";
						$extension = ".mp3";
						$att_type = 2;
					} else if (strpos($filename,".3gp") > 0 && $type=="application/octet-stream") {
						$subfolder = "/sound/sound";
						$extension = ".3gp";
						$att_type = 2;
					} else if (strpos($filename,".mp4") > 0 && $type=="application/octet-stream") {
						$subfolder = "/video/video";
						$extension = ".mp4";
						$att_type = 3;
					} else if (strpos($filename,".avi") > 0 && $type=="application/octet-stream") {
						$subfolder = "/video/video";
						$extension = ".avi";
						$att_type = 3;
		...
		for ($mi=0;$mi<sizeof($multipart);$mi++) {
    		...
    		if ($att_type > 0) {
				...
				*/
				...
				/*
				if ($lat!="" && $long!="") {
					$attachment_id=mysqli_insert_id();
